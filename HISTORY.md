...
Добавлены drawable ресурсы ic_pin_error_48.xml и ic_pin_empty_48.xml. Изменения в методах getColorMarker и getColorMarkerSmall.  
...
### 3.10
Все компоненты ListView и SimpleAdapter заменены на RecyclerView и RecyclerView.Adapter соответственно.  
...
Во все пустые блоки try catch добавлено логирование ошибок.  
...
Функция getThemeColor перенесена в файл Util.  
...
Приватные константы перенесены из файла Util в SMSmonitor.  
...
Приватные константы перенесены из файла Util в GetLocation.  
...
### 3.16.5
Приватные константы и переменные перенесены внутрь классов и объектов.  
Публичные константы перенесены между файлами.

### 3.16.6
Незначительные изменения в тестах.

### 3.16.7
Незначительные изменения в тестах.

### 3.16.8
В классе данных User тип поля id сменён с Int на Long. (Хотя, вряд ли количество контактов превысит 2147483647).  
Соответствующие изменения внесены во все функции, использующие поле id.  
В LocationActivity при генерации меню теперь используется Map для хранения соответствий полей id с типом Long и пунктов меню с типом Int.

### 3.16.9
В файлах исходного кода и ресурсов разделители \r\n заменены на \n.

### 3.16.10
Смена версии Gradle на 9.4.0.  
Переход на AGP 9.1.0.

### 3.16.11
Удалены неиспользуемые ресурсы.

### 3.16.12
Добавлено описание деревьев макетов.

### 3.16.13
Смена версии Gradle на 9.4.1.  
Обновлены версии зависимостей.

### 3.16.14
Переход на AGP 9.1.1.  
Обновлены версии зависимостей.  

Исправлена ошибка в SettingsActivity. Теперь слайдер диаметра кругов не показывается, если выбран показ координат текстом или выбрана карта OpenStreet.

### 3.16.15
Смена версии Gradle на 9.5.0.  
Переход на AGP 9.2.1.  
Смена версии compileSdk и targetSdk на 37.
...
### 5.0.0
Настроена работа через интернет-сервер.  
Активация аккаунта по одноразовой ссылке. Отправка и получение координат вручную по нажатию кнопки.  
Изменения в коде, макете и в ресурсах строк.

### 5.1.0
Настроен воркер для периодической синхронизации геолокации с интернет-сервером.  
Изменения в коде, макете и в ресурсах строк.
