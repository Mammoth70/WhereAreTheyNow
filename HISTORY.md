...
Добавлены разрешения на отправку уведомлений для работы NotificationChannel.  
Внесены соответствующие изменения в PermissionActivity (код и макет) и в код MainActivity.  
Добавлены или изменены соответствующие строковые ресурсы.

### 3.14.3
Исправлена ошибка, из-за которой темы не устанавливались при запуске приложения.
...
Теперь инициализация запроса Background Location происходит строго после одобрения Fine Location.  
...
### 3.14.8
Изменены параметры функций в классе DBHelper.  
Соответственно, в объекте DataRepository изменены вызовы этих функций.  
...
Переименован класс SMSMonitor в SMSmonitor.

### 3.14.9
Объект AppColors переименован в PinColors. Переименованы некоторые функции объекта.  
Список colors переделан в enum class Color. Соответствующим образом переделаны функции объекта PinColors и все внешние вызовы.  
...
### 3.14.11
Добавлены synchronized в функции класса DataRepository. 

### 3.15.0
Добавлены инструментальные и Unit-тесты:
- тест функции timePassed;
- инструментальный тест работоспособности тестового окружения на устройстве.  

Функция timePassed для удобства тестирования разделена собственно на timePassed (вывод текста в контекст) и calculateTimePassed (логика).  
