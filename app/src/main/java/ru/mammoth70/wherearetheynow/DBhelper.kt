...
class DBhelper(context: Context?) : SQLiteOpenHelper(context, "watnDB",
    ...
    fun addUser(phone: String?, name: String?, color: String?): Boolean {
        ...
        if (!Util.phones.contains(phone)) {
            readableDatabase.use { db ->
                val execSting = "INSERT OR IGNORE INTO users (phone, name, color) VALUES " +
                        "('" + phone + "', '" + name + "', '" + color + "');"
                try {
    ...
    fun deleteUser(id: Int): Boolean {
        ...
        if (Util.id2phone.containsKey(id)) {
            ...
            readableDatabase.use { db ->
                val execSting1 = "DELETE FROM users WHERE id = '$id';"
                val execSting2 = "DELETE FROM points WHERE phone = '$phone';"
                try {
