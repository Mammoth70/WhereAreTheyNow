# Спецификация WATN API (v5.0.0)

**Описание системы:** API для первичной настройки Android-приложения, активации устройств отправки своей геолокации и получения геолокации контактов.  
...
* **Токены (`token`, `api_token`):** Строго 64 символа. Состоят только из латинских букв от a до f в **нижнем регистре** и цифр.
* **Номера телефонов (`phone`):** Содержат только знак плюс `+` (необязателен в начале) и цифры. Пробелы, дефисы и скобки запрещены.
* **Цвета метки (`color`):** Строго HEX-формат из 6 символов в **верхнем регистре** с решеткой в начале (например, `#FF001A`).
...
#### HTTP 200 OK (Успех)
Координаты успешно приняты и записаны сервером.
```json
{
  "status": "success",
...
Внутренняя ошибка базы данных или сервера.
```json
{
  "status": "error",
...
#### HTTP 200 OK (Успех)
Данные успешно получены. Поле `data` содержит список контактов или может быть пустым массивом `[]`. Координаты передаются в виде строк.
```json
{
  "status": "success",
  "data": [
